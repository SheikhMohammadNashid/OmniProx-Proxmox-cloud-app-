...
async function ensureProxmoxCreds() {
  ...
  const response = await fetch("/api/proxmox/creds/status", {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!response.ok) {
    localStorage.removeItem("access_token");
  ...
  if (json.dry_run) {
    userMeta.textContent = `${userMeta.textContent} | DRY RUN ENABLED (no VM will be created)`;
  }
  return true;
}
...
ensureProxmoxCreds().then((ok) => {
  if (!ok) return;
  fetchMe().then(fetchJobs);
});
