...
class VMHardwareSpec(BaseModel):
    ...
    scsi_controller: str = Field(
        default="virtio-scsi-single", min_length=2, max_length=32
    )
...
class ProxmoxCredentialsUpsertRequest(BaseModel):
    base_url: str = Field(..., min_length=3, max_length=256)
    node: str = Field(..., min_length=1, max_length=64)
    token_id: str = Field(..., min_length=3, max_length=256)
    token_secret: str = Field(..., min_length=6, max_length=256)
    verify_ssl: bool = False
    dry_run: bool = True


class ProxmoxCredentialsStatusResponse(BaseModel):
    has_creds: bool
